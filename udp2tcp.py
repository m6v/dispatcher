...
"""
Пример из https://gist.github.com/giacobenin/e1534638609a0dfb17b91c81da33c023
...
class RemoteStreamProtocol(asyncio.Protocol):
    ...
    # В исходном примере в функции предполагалось получение 3-х аргументов,
    # а фактически передавалось два, что вызывало ошибку
...
# Исходнос примере эта функция не объявлялась как ассинхронная, чир вызывало ошибку
async def start_datagram_proxy(bind, port, remote_host, remote_port):
    loop = asyncio.get_event_loop()
    protocol = ProxyDatagramProtocol((remote_host, remote_port))
    # В исходном примере конечная точка создавалась по-другому
